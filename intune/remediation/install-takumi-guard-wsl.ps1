<#
.SYNOPSIS
    Takumi Guard WSL remediation (Intune Remediation - Remediation)
.DESCRIPTION
    Sets npm registry / pip index-url to Takumi Guard (anonymous, fixed values)
    inside every registered WSL distribution via "npm config set" /
    "pip config set" run in a login shell (sh -lc), so existing settings are
    preserved. A distribution that cannot run sh, and a package manager that
    is absent, cannot run, or resolves to the Windows host via interop
    (/mnt/*), is skipped.
.NOTES
...
# Package manager warnings on stderr are captured and shown only on failure.
$Configure = @(
    'set -f'
    'usable(){ p=$(command -v $1 2>/dev/null) || return 1; case $p in /mnt/*) return 1;; esac; $1 --version >/dev/null 2>&1; }'
    'if usable npm; then out=$(npm config set registry https://npm.flatt.tech/ 2>&1) || { echo RESULT:ERROR npm: $out; exit 1; }; echo OK: npm configured; else echo SKIP: npm not usable; fi'
    'pm=; if usable pip; then pm=pip; elif usable pip3; then pm=pip3; fi'
    'if [ ${pm:-none} = none ]; then echo SKIP: pip not usable; else out=$($pm config set global.index-url https://pypi.flatt.tech/simple/ 2>&1) || { echo RESULT:ERROR pip: $out; exit 1; }; echo OK: pip configured; fi'
    'echo RESULT:DONE'
) -join '; '
...
try {
    ...
    if ($distros.Count -eq 0) { Write-Output "REMEDIATION SUCCESS (no WSL distribution)"; exit 0 }

    foreach ($d in $distros) {
        $result = Invoke-WslProbe $d $Configure
        if (-not $result) { Write-Output "SKIP: $d not probeable"; continue }
        if ($result -ne "DONE") { throw "$d failed: $result" }
        Write-Output "OK: $d configured"
    }
